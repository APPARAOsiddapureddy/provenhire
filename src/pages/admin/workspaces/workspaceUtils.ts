import type { Workspace, WorkspaceDetailsDraft, WorkspaceRound, WorkspaceRoundDraft, WorkspaceStatus } from "./types";
...
export function statusBadgeClass(status: WorkspaceStatus): string {
  if (status === "draft") return "bg-slate-500/10 text-slate-700 border-slate-500/30 dark:text-slate-300";
  if (status === "published") return "bg-blue-500/10 text-blue-700 border-blue-500/30 dark:text-blue-300";
  if (status === "started") return "bg-emerald-500/10 text-emerald-700 border-emerald-500/30 dark:text-emerald-300";
  if (status === "ended") return "bg-amber-500/10 text-amber-800 border-amber-500/30 dark:text-amber-200";
...
export function detailsFromWorkspace(workspace: Workspace): WorkspaceDetailsDraft {
  return {
    name: workspace.name,
...
export function parseIntegerDraft(value: string, min: number, max: number): number | null {
...
function roundToDraft(round: WorkspaceRound | WorkspaceRoundDraft): WorkspaceRoundDraft {
  return {
    ...round,
...
export function normalizeRounds(rounds: Array<WorkspaceRound | WorkspaceRoundDraft> | undefined, totalRounds: number): WorkspaceRoundDraft[] {
  const byOrder = new Map((rounds ?? []).map((round) => [round.order, roundToDraft(round)]));
  return Array.from({ length: totalRounds }, (_, index) => {
    const order = index + 1;
    return byOrder.get(order) ?? {
      ...defaultRound(order),
      scoreWeightage: String(Math.floor(100 / totalRounds) + (order <= 100 % totalRounds ? 1 : 0)),
    };
  });
}
...
export function roundsToPayload(rounds: WorkspaceRoundDraft[]): WorkspaceRound[] | null {
...
export function validateWorkspaceDetails(details: WorkspaceDetailsDraft): string | null {
  if (!details.name.trim()) return "Workspace name is required.";
  if (!details.organization.trim()) return "Organization is required.";
  if (!details.startAt || !details.endAt) return "Start and end time are required.";
  if (new Date(details.startAt).getTime() >= new Date(details.endAt).getTime()) {
...
export function validateRounds(rounds: WorkspaceRoundDraft[], totalRounds: number): string | null {
  if (rounds.length !== totalRounds) return `Configure exactly ${totalRounds} round${totalRounds === 1 ? "" : "s"}.`;
  const payload = roundsToPayload(rounds);
  if (!payload) return "Round numeric fields must be valid whole numbers.";
  const weightTotal = payload.reduce((sum, round) => sum + round.scoreWeightage, 0);
  ...
  for (const round of payload) {
    ...
    if (round.questionCount < 1) return `Round ${round.order} needs at least 1 question.`;
    if (round.timeLimitMins < 1) return `Round ${round.order} needs a positive time limit.`;
    const difficultyTotal = round.easyCount + round.mediumCount + round.hardCount;
...
export function canArchive(workspace?: Pick<Workspace, "status"> | null): boolean {
  return !!workspace && workspace.status !== "archived" && workspace.status !== "draft";
}

export function canEditDraft(workspace?: Pick<Workspace, "status"> | null): boolean {
