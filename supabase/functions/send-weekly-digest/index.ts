...
serve(async (req) => {
  ...
  try {
    ...
    // Check if it's the service role key or anon key with proper authorization
    ...
    console.log("Starting weekly job digest processing...");
    ...
    if (!recentJobs || recentJobs.length === 0) {
      console.log("No new jobs in the past week");
    ...
    console.log(`Found ${recentJobs.length} jobs from the past week`);
    let emailsSent = 0;
    ...
    for (const sub of subscriptions) {
      ...
      console.log(`Sending digest with ${matchingJobs.length} jobs to ${sub.email}`);
      ...
      const emailContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; text-align: center;">
            <h1 style="color: white; margin: 0;">Your Weekly Job Digest 📬</h1>
            <p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0; font-size: 16px;">
              ${matchingJobs.length} job${matchingJobs.length > 1 ? 's' : ''} matching your skills this week
            </p>
          </div>
          <div style="padding: 30px; background: #f9fafb;">
            <p style="font-size: 15px; color: #374151; margin-bottom: 24px;">
              Here are the best job opportunities from the past week based on your skill profile:
            </p>
            ...
            <p style="font-size: 13px; color: #9ca3af; text-align: center; margin-top: 30px; border-top: 1px solid #e5e7eb; padding-top: 20px;">
              You're receiving this weekly digest because you subscribed to job alerts on ProvenHire.<br>
      ...
      const emailResponse = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          to: [sub.email],
          subject: `📬 Your Weekly Digest: ${matchingJobs.length} matching job${matchingJobs.length > 1 ? 's' : ''}`,
          html: emailContent,
        }),
      });
    ...
    console.log(`Weekly digest completed. Sent ${emailsSent} emails.`);
