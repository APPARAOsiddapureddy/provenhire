...
**After AI interview (v2):** Completing the session sets `Interview.status` to **`completed`**, runs **`evaluateFullInterviewMultiPass`** (three parallel **`evaluateFullInterview`** calls, merged scores and strictest claim-credibility risk), persists **`evaluationPassCount`** and **`evaluationScoreVariance`**, then **`recordAiInterviewSubmittedForAdminReview`**. Verification stage **`expert_interview`** is set **`completed`** (with score); **`AdminReviewQueue`** is created/updated (**`pending`**) for Human Expert routing. **Stage 5 (Human Expert)** unlocks when (a) the **employer** selects **`human_expert`** via **`PATCH /api/jobs/recruiter/applications/:applicationId/next-interview`** (**[PRD_RECRUITER.md](PRD_RECRUITER.md)** §7.6 — **`approveAdminReviewQueueForHumanExpert`**), or (b) an **admin** **`POST /api/admin/ai-interview-queue/:id/approve`** (same unlock path; may waive first paid attempt). **`.../reject`** sets queue **`rejected`** (**`failed`** from candidate POV for that gate). If the employer picks **provenhire_ai** or **company_employee**, the queue may be set to **`recruiter_redirected`** — Human Expert stays locked until a later approve path. Learners see scores in **`VerificationFlow`**; Human Expert booking follows the above gates.
...
| **TTS (system design stages)** | Typed LLD/HLD sessions still **speak** prompts | Shared **`src/lib/interviewTts.ts`** (`speakText`, handles MP3 and JSON fallback, **`unlockInterviewAudioOutput`**) — **`SystemDesignInterviewStage.tsx`**, **`DataSystemDesignStage.tsx`**. |
...
**Admin (human gate):** `GET /api/admin/ai-interview-queue/pending`, `POST /api/admin/ai-interview-queue/:id/approve`, `POST .../reject` — see **`humanInterviewGate.service.ts`**. **Recruiter gate:** **`PATCH /api/jobs/recruiter/applications/:applicationId/next-interview`** — §7.6 in **[PRD_RECRUITER.md](PRD_RECRUITER.md)**.
...
- **Client capture (v2):** **`ExpertInterviewStage`** uses **`useProctoringRiskMonitor`** (tab/visibility + optional lightweight vision on **`proctorVideoRef`**); events POST to **`/api/proctoring/alerts`** with **`testType: "ai_interview"`** and **`sessionId`** = interview id. **System design** stages use the same hook with **`testType`** **`"system_design"`** or **`"data_system_design"`** (increments **`Interview.proctoringEventCount`** via **`proctoring`** routes).
...
| **Human gate** | **`recordAiInterviewSubmittedForAdminReview`**: queue email; **`expert_interview`** **`completed`**; unlock via **recruiter** **`PATCH`** (**`approveAdminReviewQueueForHumanExpert`**) or **admin** approve; queue status **`recruiter_redirected`** when employer picks non–Human-Expert path. |
...
| **Employer gate + system design (April 2026)** | Part A §2 after-AI flow: **`expert_interview`** **`completed`** + **`AdminReviewQueue`** + recruiter **`PATCH`** / admin approve; §5 **`interviewTts`** on system design stages; §8 proctoring **`testType`** for software/data system design; §6 recruiter route cross-link. |
