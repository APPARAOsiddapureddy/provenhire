...
export async function sendSkillExpiryReminderEmail(
  ...
  const { error } = await resend.emails.send({
    ...
    html: `
      ...
      <p><a href="${process.env.BASE_URL || "https://provenhire.in"}/dashboard/jobseeker" style="color:#D4AF37;font-weight:bold">Reattempt Verification →</a></p>
      <p>— The ProvenHire Team</p>
    `,
  });
  return !error;
...
const baseUrl = () => process.env.BASE_URL || "https://provenhire.in";
